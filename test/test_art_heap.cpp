...
TEST_F(HeapArtTest, ScanRange) {
  TestHeap heap;
  heap_db db{heap};

  // Insert keys 0..9.
  std::vector<std::array<std::byte, 8>> key_store;
  key_store.reserve(10);
  ...
  // scan_range [3, 7) should yield values 3, 4, 5, 6.
  const auto from_key = unodb::key_view{key_store[3].data(), 8};
  const auto to_key = unodb::key_view{key_store[7].data(), 8};
  std::vector<std::uint64_t> results;
  db.scan_range(from_key, to_key, [&results](auto visitor) {
    results.push_back(visitor.get_value());
    return false;
  });
  const std::vector<std::uint64_t> expected{3, 4, 5, 6};
  EXPECT_EQ(results, expected);
}

TEST_F(HeapArtTest, ScanRangeReverse) {
  TestHeap heap;
  heap_db db{heap};
  ...
  // scan_range [7, 3) — from > to means reverse, yielding 7, 6, 5, 4.
  const auto from_key = unodb::key_view{key_store[7].data(), 8};
  const auto to_key = unodb::key_view{key_store[3].data(), 8};
  std::vector<std::uint64_t> results;
  db.scan_range(from_key, to_key, [&results](auto visitor) {
    results.push_back(visitor.get_value());
    return false;
  });
  const std::vector<std::uint64_t> expected{7, 6, 5, 4};
  EXPECT_EQ(results, expected);
}

TEST_F(HeapArtTest, ScanFromWithKey) {
  TestHeap heap;
  heap_db db{heap};
  ...
  // scan_from key 5 forward, verify get_key() works.
  const auto from_key = unodb::key_view{key_store[5].data(), 8};
  std::vector<std::uint64_t> results;
  db.scan_from(from_key, [&results, &key_store](auto visitor) {
    results.push_back(visitor.get_value());
    // Verify get_key() returns the correct key.
    const auto k = visitor.get_key();
    const auto& expected_key = key_store[visitor.get_value()];
    EXPECT_EQ(k.size(), expected_key.size());
    if (k.size() == expected_key.size()) {
      EXPECT_EQ(std::memcmp(k.data(), expected_key.data(), k.size()), 0);
    }
    return false;
  });
  const std::vector<std::uint64_t> expected{5, 6, 7, 8, 9};
  EXPECT_EQ(results, expected);
}
